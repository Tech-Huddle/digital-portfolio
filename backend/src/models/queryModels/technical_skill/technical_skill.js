...
exports.technical_skillCreate = async (params, next) => {
    logger.info("*** Starting %s of %s ***", getName().functionName, getName().fileName);
    try {

        var params = params.data;
        let result = "";
        let get_user_no = await db.TechnicalSkill.findOne({ where: { "user_id": params.user_id } });
        if (get_user_no == null) {
            result = await db.TechnicalSkill.create(params);
        } else {
...
exports.technical_skillList = async (params, next) => {
    logger.info("*** Starting %s of %s ***", getName().functionName, getName().fileName);
    try {
        params = params.data;
        var inner_hash = {};
        ...
        var result = await db.TechnicalSkill.findAll(findAll_hash);
        if (!params.filter || (params.filter && (!params.filter.total || parseInt(params.filter.total) == 0))) {
            ...
            var count = await db.TechnicalSkill.findAll(findAll_hash);
            logger.info("*** Ending %s of %s ***", getName().functionName, getName().fileName);
...
exports.technical_skillUpdate = async (params, next) => {
    logger.info("*** Starting %s of %s ***", getName().functionName, getName().fileName);
    try {
        var id = params.id;
        var data = params.data;
        var dataExists = await db.TechnicalSkill.findOne({ where: { id: id } });
        dataExists = JSON.parse(JSON.stringify(dataExists));
        if (dataExists != null) {
            let updateData = await db.TechnicalSkill.update(
                data, {
                where: { id: id }
            });
            if (updateData[0] == 0) {
...
exports.technical_skillDelete = async (params, next) => {
    logger.info("* Starting %s of %s *", getName().functionName, getName().fileName);
    try {
        var id = params.data;
        var dataExists = await db.TechnicalSkill.findOne({
            where: { id: id }
        });
        if (dataExists != null) {
            var result = await db.TechnicalSkill.destroy({
                where: { id: id }
            });
