...
exports.EducationDetailsList = async (params, next) => {
    ...
    try {
        ...
        var result = await db.EducationDetails.findAll(findAll_hash);
        if (!params.filter || (params.filter && (!params.filter.total || parseInt(params.filter.total) == 0))) {
            ...
            var count = await db.EducationDetails.findAll(findAll_hash);
            logger.info("*** Ending %s of %s ***", getName().functionName, getName().fileName);
...
exports.EducationDetailsUpdate = async (params, next) => {
    ...
    try {
        ...
        var dataExists = await db.EducationDetails.findOne({ where: { id: id } });
        dataExists = JSON.parse(JSON.stringify(dataExists));
        if (dataExists != null) {
            let updateData = await db.EducationDetails.update(
                data, {
                where: { id: id }
            });
            if (updateData[0] == 0) {
...
exports.EducationDetailsDelete = async (params, next) => {
    ...
    try {
        var id = params.data;
        var dataExists = await db.EducationDetails.findOne({
            where: { id: id }
        });
        if (dataExists != null) {
            var result = await db.EducationDetails.destroy({
                where: { id: id }
            });
