...
exports.certificationCreate = async (params, next) => {
    logger.info("*** Starting %s of %s ***", getName().functionName, getName().fileName);
    try {

        var params = params.data;
        let result = "";
        let get_user_no = await db.Certification.findOne({ where: { "user_id": params.user_id } });
        if (get_user_no == null) {
            result = await db.Certification.create(params);
        } else {
...
exports.certificationList = async (params, next) => {
    logger.info("*** Starting %s of %s ***", getName().functionName, getName().fileName);
    try {
        params = params.data;
        var inner_hash = {};
        ...
        var result = await db.Certification.findAll(findAll_hash);
        if (!params.filter || (params.filter && (!params.filter.total || parseInt(params.filter.total) == 0))) {
            ...
            var count = await db.Certification.findAll(findAll_hash);
            logger.info("*** Ending %s of %s ***", getName().functionName, getName().fileName);
...
exports.certificationUpdate = async (params, next) => {
    logger.info("*** Starting %s of %s ***", getName().functionName, getName().fileName);
    try {
        var id = params.id;
        var data = params.data;
        var dataExists = await db.Certification.findOne({ where: { id: id } });
        dataExists = JSON.parse(JSON.stringify(dataExists));
        if (dataExists != null) {
            let updateData = await db.Certification.update(
                data, {
                where: { id: id }
            });
            if (updateData[0] == 0) {
...
exports.certificationDelete = async (params, next) => {
    logger.info("* Starting %s of %s *", getName().functionName, getName().fileName);
    try {
        var id = params.data;
        var dataExists = await db.Certification.findOne({
            where: { id: id }
        });
        console.log(dataExists)
        if (dataExists != null) {
            var result = await db.Certification.destroy({
                where: { id: id }
            });
            return ({ "message": "Deleted Successfully", "success": true })
        } else {
            console.log("@@@@")
            return({ "message": "Data not found", "success": false, "status": 404 });
        }
    } catch (error) {
        logger.error("* Error in %s of %s *", getName().functionName, getName().fileName);
        logger.error(error.message || JSON.stringify(error));
